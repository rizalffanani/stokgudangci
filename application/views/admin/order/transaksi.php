<form action="<?= base_url('pesanantamu/checkout')?>" method="post" class='box'>
    <?php 
        ...
        $data = $this->db->get('t_barang')->result(); 
    ?>
    <select class="form-control" id="menu" onchange="tambah(this.value)">
        <?php 
            echo "<option value='0'>pilih</option>";
            foreach ($data as $d){
                if (!in_array($d->id_barang, $datas)) {
                    echo "<option value='".$d->id_barang."'>".strtoupper($d->nama_barang)."</option>";
                }
            }
        ?>
    </select><br>
    	    ...
    	    <tbody id="table">
                ...
                <tr>
                    <td>
                        ...
                        <input type="hidden" name="kategori[]" class="form-control" value="<?= $key['kategori']?>"/>
...
<script type="text/javascript">
    ...
    function tambah (a) {
        if (a>0) {
            ...
            $.ajax({
                type:"POST",
                url:"<?=site_url('admin/transaksi/tabel');?>", 
                data : {a:hitung,b:a},  
    ...
    function hapus2(a,b) {
        ...
        $.ajax({
            type:"POST",
            url:"<?=site_url('admin/transaksi/hapus_cart');?>", 
            dataType : "JSON",
            data : {row_id:b},
            success: function(data){
                var x = document.getElementById("menu");
    ...
    function update(a,b) {
        $.ajax({
          type:"GET",
          url:"<?=site_url('admin/transaksi/update_cart');?>/"+a+'/'+b, 
          dataType : "JSON",   
          success: function(data){   
            document.getElementById('totals').value = data.nilai;   
          }  
